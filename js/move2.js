...
let is_Divided ; // 0 is none, 1 is yes
...
function RandomPostion(arr1, arr2) {
    ...
    PosFlag = {
        'Dorm': new Position('Dorm', [0.9 * 20, 20 * 0.9], [0.9 * 200, 100 * 0.9], 0.01),
        'Supermarket': new Position('Supermarket', [0.9 * 20, 140 * 0.9], [0.9 * 200, 100 * 0.9], 0.005),
        'Classroom': new Position('Classroom', [0.9 * 20, 260 * 0.9], [0.9 * 200, 100 * 0.9], 0.007),
        'Canteen': new Position('Canteen', [0.9 * 240, 20 * 0.9], [0.9 * 200, 100 * 0.9], 0.008),
        'Playground': new Position('Playground', [0.9 * 240, 140 * 0.9], [0.9 * 200, 100 * 0.9], 0.0005),
        'Library': new Position('Library', [0.9 * 240, 260 * 0.9], [0.9 * 200, 100 * 0.9], 0.006)
    }
    PosFlag['Dorm'].initRoom();
    PosFlag['Classroom'].initClass();
    Divide = new Position('隔离区', [0.9 * 20, 380 * 0.9], [0.9 * 420, 100 * 0.9], 0)
            ...
            if (PosFlag[i].s > 0 && Expose > 0) {
                ...
                if (Expose >= PosFlag[i].s) {
                    for (let j = 0; j < PosFlag[i].s; ++j) {
                        ...
                        PosFlag[i].sPos[j].passage = PosFlag[i].sPos[j].parent.passage + 3;
                    }
                } else {
                    while (Expose > 0) {
                        if (Expose <= 0) break;
                        ...
                        PosFlag[i].sPos[j].mark++;
                        PosFlag[i].sPos[j].time = randomNum(1, 14);
                        let pp = randomNum(0, PosFlag[i].ePos.length - 1);
                        console.log(pp)
                        PosFlag[i].sPos[j].parent = PosFlag[i].ePos[pp];
                        // console.log(PosFlag[i].sPos[j])
                        PosFlag[i].sPos[j].passage = PosFlag[i].sPos[j].parent.passage + 3;
                        Expose--;
                    }
                    // }
...
for (let i in PersonData) {
    net_work_data.nodes.push({
        ...
        "size": 1 / PersonData[i].passage * 100,
