...
function closeNet(posNum) {
    close_data.nodes.push({
        ...
        "size": PersonData[posNum].passage + 1,
    ...
    for (let i in PersonData[posNum].closeContact) {
        ...
        close_data.nodes.push({
            ...
            "size": contactPeo.passage + 1,
            "id": contactPeo.name,
            "infectplace": contactPeo.infectPlace,
            "infectTime": contactPeo.InfectTime
        });
        close_data.links.push({
            "source": PersonData[posNum].name,
            "value": 1,
            "target": contactPeo.name,
            "infectTime": contactPeo.InfectTime
        });
    }
}