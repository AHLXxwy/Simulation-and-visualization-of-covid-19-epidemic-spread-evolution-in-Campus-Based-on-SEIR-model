...
var colors = ['#00FFFFFF', '#FFD700', 'red', '#00FF00']
...
let closeContactTimeLine = new Array();
...
function s() {
    ...
    closeContactTimeLine = new Array();
    ...
    PersonData[0].mark = 1;
    PersonData[0].time = randomNum(1, incubation) * 48;
    ...
    for (let cnt = 1; cnt < 720; ++cnt) {
        // if (cnt == 200)
        let closePeopleData = new Array();
        let sumS = 0;
            ...
            for (var i in PosFlag) {
                ...
                for (let k in PosFlag[i].ePos) {
                    ...
                    for (let j in PosFlag[i].people) {
                        ...
                        let dis = calculateDistance(PosFlag[i].people[j].nowPos, PosFlag[i].ePos[k].nowPos);
                        if (dis > touchDistance)
                            continue;
                        risk[PosFlag[i].people[j].name]++;
                        ...
                        if (PosFlag[i].people[j].mark == 0) {
                            ...
                            if (PosFlag[i].people[j].mark == 1) {
                                ...
                                PosFlag[i].people[j].time = randomNum(1, incubation) * 48 + 1;
                            }
                        }
                        let tempData = {
                            id: PosFlag[i].people[j].name,
                            hour: cnt,
                            dis: (dis / touchDistance).toFixed(1),
                            risk: risk[PosFlag[i].people[j].name],
                            status: PosFlag[i].people[j].mark,
                            infect_name: PosFlag[i].ePos[k].name
                        }
                        closePeopleData.push(tempData);
                    }
                ...
                for (let k in PosFlag[i].iPos) {
                    ...
                    for (let j in PosFlag[i].people) {
                        // let j = randomNum(0, PosFlag[i].people.length - 1);
                        let dis = calculateDistance(PosFlag[i].people[j].nowPos, PosFlag[i].iPos[k].nowPos);
                        if (dis > touchDistance)
                            continue;
                        risk[PosFlag[i].people[j].name]++;
                        contact.push(PosFlag[i].people[j]);
                        ...
                        let tempData = {
                            ...
                            risk: risk[PosFlag[i].people[j].name],
                            status: PosFlag[i].people[j].mark,
                            infect_name: PosFlag[i].iPos[k].name
                        }
                        closePeopleData.push(tempData);
                    }
                    let id_group = new Array();
        ...
        closeContactTimeLine.push(closePeopleData);
        // console.log("closematrix:")
...
function start(cntx) {
    ...
    Circlesvg1.selectAll('#Circle')
        ...
        .attr('cx', (d, i) => {
            // return d.nowPos[0];
            if (typeof (d.path[cntx]) != 'undefined')
